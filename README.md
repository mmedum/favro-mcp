...
| `favro_set_card_custom_field` | 5 | Mutating. Sets a single custom-field value on a card. Phase 5.5 supports the simple types: Text, Number, Date, Checkbox, Single select. Pass exactly one of `text` / `number` / `date` / `checkbox` / `single_select_item_id` matching the resolved field's Type. Long-tail types (Members, Status, Multi-select, Rating, Link) return a typed error pointing to Phase 7. Live success invalidates the search-cards cache. Pass `dry_run: true` to preview. |
