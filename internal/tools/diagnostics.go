// Package tools is the MCP surface: one registration per tool, its
// input and output schemas, and the wrapper every tool goes through.
//
// A file here holds the tools for one area of Favro and as little logic
// as the tool can get away with — anything that resolves, searches,
// fans out or decides a policy belongs in internal/service, so that
// what a tool does stays visible in the shape of its handler.
package tools
...
type RateLimitOutput struct {
	...
	ThrottleDelaySecs float64 `json:"throttle_delay_seconds" jsonschema:"seconds Favro held the last response back to let the token bucket refill (X-RateLimit-Delay); non-zero means the call succeeded but the account is being throttled. Favro fails the request outright once the needed delay would exceed 10 seconds."`
...
func registerRateLimitStatus(reg *registry, client *favroapi.Client) {
	addTool(reg, &mcp.Tool{
		...
		Description: "Reports the most recently observed Favro rate-limit headers " +
			...
			"A non-zero `throttle_delay_seconds` is the early warning: Favro is already " +
			"stalling responses to refill the token bucket, and starts rejecting with 429 " +
			"once the needed stall would exceed 10 seconds.",
		...
		out := RateLimitOutput{
			...
			ThrottleDelaySecs: snap.Delay.Seconds(),
			LastPath:          snap.Path,
