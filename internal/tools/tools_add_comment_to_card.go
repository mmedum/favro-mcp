...
var errAddCommentSearchScopeRequired = render.Sentinel(render.ClassInvalid, "favro_add_comment_to_card: search_query requires exactly one of widget_common_id or collection_id")

// errAddCommentIdentityRequired is returned when the caller supplies
// neither a card identity nor a search_query.
var errAddCommentIdentityRequired = render.Sentinel(render.ClassInvalid, "favro_add_comment_to_card: pass exactly one of card_common_id, card_id, sequential_id, or search_query")

// errAddCommentNoMatch is returned when search_query matches no
// cards in the supplied scope.
var errAddCommentNoMatch = render.Sentinel(render.ClassNotFound, "favro_add_comment_to_card: search_query matched no cards in the supplied scope")
...
type addCommentToCardResult struct {
	...
	Candidates []service.SearchedCard `json:"candidates,omitempty" jsonschema:"ranked candidate cards when ambiguous=true"`
}

func registerAddCommentToCard(reg *registry, r *service.Resolver) {
...
func resolveAddCommentTarget(ctx context.Context, r *service.Resolver, in *addCommentToCardInput) (cardCommonID string, ambiguous bool, candidates []service.SearchedCard, err error) {
	idCount := 0
	if in.CardCommonID != "" {
		idCount++
	}
...
func resolveAddCommentBySearch(ctx context.Context, r *service.Resolver, in *addCommentToCardInput) (string, bool, []service.SearchedCard, error) {
...
func pickSearchScope(widgetCommonID, collectionID string) (service.SearchScope, string, bool) {
	switch {
	case widgetCommonID != "" && collectionID == "":
		return service.SearchScopeWidget, widgetCommonID, true
	case widgetCommonID == "" && collectionID != "":
		return service.SearchScopeCollection, collectionID, true
	default:
		return 0, "", false
	}
}
