...
func registerResolveCustomField(reg *registry, r *service.Resolver) {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in resolveCustomFieldInput) (*mcp.CallToolResult, resolveOutput[service.ResolvedCustomField], error) {
		matches, cached, err := r.ResolveCustomField(ctx, in.Name, in.Limit, in.ForceRefresh)
		if err != nil {
			return nil, resolveOutput[service.ResolvedCustomField]{}, err
		}
		return nil, resolveOutput[service.ResolvedCustomField]{Candidates: matches, Cached: cached}, nil
	})
}
