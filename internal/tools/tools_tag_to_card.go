...
var errTagToCardUnknown = render.Sentinel(render.ClassNotFound, "favro: tag name not found in active organization (typo? to add a brand-new tag, call favro_create_tag explicitly)")
...
var errTagToCardAmbiguous = render.Sentinel(render.ClassAmbiguous, "favro: multiple tags share this exact name; pick one via favro_resolve_tag and use favro_update_card with add_tag_ids / remove_tag_ids directly")
...
func registerAddTagToCard(reg *registry, r *service.Resolver) {
...
func registerRemoveTagFromCard(reg *registry, r *service.Resolver) {
	addTool(reg, &mcp.Tool{
...
func resolveExactTagID(ctx context.Context, r *service.Resolver, tagName string) (string, error) {
	if tagName == "" {
