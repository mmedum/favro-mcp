...
func registerAppendCardDescription(reg *registry, r *service.Resolver) {
	addTool(reg, &mcp.Tool{
		...
		newBody := service.AppendDescription(oldBody, in.Text)
...
func registerPrependCardDescription(reg *registry, r *service.Resolver) {
	addTool(reg, &mcp.Tool{
		...
		oldBody := card.DetailedDescription
		newBody := service.PrependDescription(oldBody, in.Text)
...
func registerReplaceInCardDescription(reg *registry, r *service.Resolver) {
	addTool(reg, &mcp.Tool{
		...
		newBody, hits, err := service.ReplaceInDescription(oldBody, in.Find, in.Replace, count, in.UseRegex)
		if err != nil {
			return nil, writeOutput[service.EditorResult]{}, err
		}
		if hits == 0 {
			return nil, writeOutput[service.EditorResult]{}, fmt.Errorf("%w (card %q)", errDescriptionFindNoMatch, in.CardID)
		}
...
func runDescriptionEdit(
...
) (writeOutput[service.EditorResult], error) {
	result := service.NewEditorResult(cardID, oldBody, newBody)
	writeCtx := ctx
	if dryRun {
	...
	if err == nil {
		...
		return writeOutput[service.EditorResult]{Result: &result}, nil
	}
	...
	if errors.As(err, &rec) {
		...
		return writeOutput[service.EditorResult]{
			DryRun:             true,
			Result:             &result,
	...
	return writeOutput[service.EditorResult]{}, err
}
