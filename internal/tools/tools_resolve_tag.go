...
func registerResolveTag(reg *registry, r *service.Resolver) {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in resolveTagInput) (*mcp.CallToolResult, resolveOutput[service.ResolvedTag], error) {
		matches, cached, err := r.ResolveTag(ctx, in.Name, in.Limit, in.ForceRefresh)
		if err != nil {
			return nil, resolveOutput[service.ResolvedTag]{}, err
		}
		return nil, resolveOutput[service.ResolvedTag]{Candidates: matches, Cached: cached}, nil
	})
}
