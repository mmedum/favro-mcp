...
func registerResolveColumn(reg *registry, r *service.Resolver) {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in resolveColumnInput) (*mcp.CallToolResult, resolveOutput[service.ResolvedColumn], error) {
		matches, cached, err := r.ResolveColumn(ctx, in.WidgetCommonID, in.Name, in.Limit, in.ForceRefresh)
		if err != nil {
			return nil, resolveOutput[service.ResolvedColumn]{}, err
		}
		return nil, resolveOutput[service.ResolvedColumn]{Candidates: matches, Cached: cached}, nil
	})
}
