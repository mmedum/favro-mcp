...
// the caller's intent.
var errSearchCardsScopeConflict = render.Sentinel(render.ClassInvalid, "favro_search_cards: pass widget_common_id OR collection_id, not both")
...
var errSearchCardsScopeRequired = render.Sentinel(render.ClassInvalid, "favro_search_cards: widget_common_id or collection_id is required (Favro's /cards endpoint rejects unfiltered listings); call favro_resolve_widget or favro_resolve_collection first if you don't know one")
...
type searchCardsOutput struct {
	Results []service.SearchedCard `json:"results" jsonschema:"ranked search hits; empty when nothing matches"`
...
func registerSearchCards(reg *registry, r *service.Resolver) {
	addTool(reg, &mcp.Tool{
		...
		var scope service.SearchScope
		var scopeID string
		switch {
		case in.WidgetCommonID != "":
			scope = service.SearchScopeWidget
			scopeID = in.WidgetCommonID
		case in.CollectionID != "":
			scope = service.SearchScopeCollection
