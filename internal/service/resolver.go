...
// Client is the Favro client this Resolver was built on.
//
// The tool layer reaches through it for the plain CRUD calls that need
// no resolution, so that a tool takes one dependency rather than two
// and cannot be handed a client and a resolver built on different
// credentials.
func (r *Resolver) Client() *favroapi.Client { return r.client }

// NewResolver builds a Resolver wrapping the given client. Caches
// start empty.
func NewResolver(client *favroapi.Client) *Resolver {
...
// The page walk delegates to favroapi.Paginate so the requestID
...
func listAllCached[T any](
	...
	var all []T
	if err := favroapi.Paginate(ctx, client, path, query, func(env favro.PageEnvelope[T]) error {
...
// The score ladder. Exported because every favro_resolve_* tool
// description quotes it to the model, and the description lives in
// another package now — so these are the one source of truth for both
// the number a caller sees and the number it gets.
const (
	ScoreExact     = 1.0
	ScorePrefix    = 0.7
	ScoreSubstring = 0.4
...
// ListAllTags returns every tag in the organization, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
...
// InvalidateTagCache drops the tag cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
...
// listAllUsers returns every user the token can see, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
// answer from a fresh one.
...
// invalidateUserCache drops the user cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
...
// listAllCollections returns every collection the token can see, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
// answer from a fresh one.
...
// InvalidateCollectionCache drops the collection cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
...
// listAllWidgets returns every widget the token can see, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
// answer from a fresh one.
...
// InvalidateWidgetCache drops the widget cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
...
var errMissingResolveWidgetCommonID = render.Sentinel(render.ClassInvalid, "favro: widget_common_id is required for resolving columns")

// listColumnsForWidget returns one widget's columns, cached per
// widget. Column names repeat across widgets, so there is no
// org-wide column list to cache instead.
...
// InvalidateColumnCache drops one widget's column cache. A write tool
// calls this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing. It
// takes the widget because the cache is per widget: column names repeat
// across widgets, so there is no org-wide list to drop instead.
...
// ListAllCustomFields returns every custom field the token can see, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
// answer from a fresh one.
...
// invalidateCustomFieldCache drops the custom-field cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
...
// listAllGroups returns every group the token can see, from the
// cache when one is warm. The second return says whether it came
// from there, so a caller that has just written can tell a stale
// answer from a fresh one.
...
// InvalidateGroupCache drops the group cache. A write tool calls
// this on success, or the next resolve answers from before the
// write — which is the failure CLAUDE.md names as load-bearing.
