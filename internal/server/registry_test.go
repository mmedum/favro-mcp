...
func listToolsWith(t *testing.T, opts Options) map[string]*mcp.Tool {
	...
	cs := connectInMemoryOpts(t, favro.NewClient(fixtureToken()), opts)
...
// TestEverySentinelIsClassified reads this package's source for
// package-level error sentinels and requires each one to name its
// class.
//
// Reading the source is the point. render.Classify falls back to
// ClassInvalid, which is right for every unsentinelled error here
// today and wrong the moment somebody adds a sentinel that means
// "not found" — and a fallback is exactly the kind of thing that
// silently absorbs a mistake. Go cannot enumerate a package's
// variables at run time, so this parses them.
		...
		for _, decl := range file.Decls {
			...
			for _, spec := range gen.Specs {
				...
				for i, name := range vs.Names {
					...
					require.True(t, ok,
						"%s: sentinel %s is not built by classed()", path, name.Name)
					fn, ok := call.Fun.(*ast.Ident)
					require.True(t, ok && fn.Name == "classed",
						"%s: sentinel %s must be classed(render.Class…, …) so the boundary can render [class] without matching on its text",
						path, name.Name)
